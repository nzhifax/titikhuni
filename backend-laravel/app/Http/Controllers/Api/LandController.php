...
class LandController extends Controller
{
    ...
    public function getRiskAnalysis(string $id): JsonResponse
    {
        ...
        foreach ($tables as $key => $table) {
            // Transform the input coordinates (SRID 4326) dynamically to match each hazard table's SRID
            $result = DB::selectOne("
                SELECT gridcode FROM \"$table\"
                WHERE ST_Intersects(geom, ST_Transform(ST_SetSRID(ST_MakePoint(:lng, :lat), 4326), ST_SRID(geom)))
                LIMIT 1
            ", ['lng' => $lng, 'lat' => $lat]);
